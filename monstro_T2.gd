...
const armadura_monstro = 4#--------------------------ARMADURA DO MONSTRO PARA PODER TOMAR HIT
const monstro_hp = 5#--------------------------------QUANTIDADE DE HITS QUE MONSTRO DEVE RECEBER PARA MORRER
const dano = 18#--------------------------------------DANO QUE MONSTRO CAUSA NO KNIGHT(PORCENTAGEM)
...
const timer_entre_ataques = 0.2#-----------------------TEMPO DE ESPERA ENTRE ATAQUES
...
const delay_ultimo_sprite = 0.2#------TEMPO NO ULTIMO SPRITE(SPRITE DE PAUSA)
...
func altera_timer(delay_ataque, delay_do_hit):
	timer_ate_hitar.set_wait_time(delay_ataque)
	tempo_hit_monstro = delay_do_hit
...
func monstro_ataque_1():
	if !monstro_atacando:
		...
		if !perfect_parry and armadura_atual > 0:
			...
			vulneravel()
		$area_ataque/shape_ataque.disabled = true
	#-----------------MINI DELAY QUANDO KNIGHT DEFENDER ATAQUE COM SUCESSO-----------------
