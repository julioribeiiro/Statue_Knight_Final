...
const armadura_monstro = 3#--------------------------ARMADURA DO MONSTRO PARA PODER TOMAR HIT
const monstro_hp = 8#--------------------------------QUANTIDADE DE HITS QUE MONSTRO DEVE RECEBER PARA MORRER
var dano    #--------------------------------------DANO QUE MONSTRO CAUSA NO KNIGHT(PORCENTAGEM)
const stamina_recebida_ao_matar = 10#----------------STAMINA QUE KNIGHT RECEBE AO MATAR O MONSTRO(PORCENTAGEM)
...
const delay_knight_defendeu = 0.05 #---------------TEMPO QUE MONSTRO FICA PARADO APOS DEFENDER ATAQUE COM SUCESSO

#-----ATAQUE 1 MONSTRO----------------
const delay_ataque_1_aviso_1 = 0.75
const delay_ataque_1_pausa_1 = 0.25
const delay_ataque_1_hit_1 = 0.15
const dano_1_ataque_1 = 25
const delay_ataque_1_pausa_2 = 0.5


#-----ATAQUE 2 MONSTRO----------------
const delay_ataque_2_aviso_1 = 0.5
const delay_ataque_2_pausa_1 = 0.5
const delay_ataque_2_hit_1 = 0.1
const dano_1_ataque_2 = 15
const delay_ataque_2_pausa_2 = 0.2
const delay_ataque_2_hit_2 = 0.22
const dano_2_ataque_2 = 20
const delay_ataque_2_pausa_3 = 0.5
...
func monstro_ataque_1():
	if !monstro_atacando:
		altera_timer(dano_1_ataque_1, delay_ataque_1_pausa_1, delay_ataque_1_hit_1)
		$Label.text = "Ataque 1 Padrao 1"
		monstro_atacando = true
		$sprites_monstro.frame = 5#------------------AVISO--------
		yield(get_tree().create_timer(delay_ataque_1_aviso_1), "timeout")#----VIRA 2 SEGUNDOS
		$sprites_monstro.frame = 6#-----------------ESPERA------------------
		timer_ate_hitar.start()#-------------COMECA O DELAY_ATAQUE_1_PAUSA_1
		yield(timer_ate_hitar, "timeout")
		$sprites_monstro.frame = 1#-----------------MONSTRO HITA
		$fire.play()
		perfect_parry_desativado = true
		yield(get_tree().create_timer(3*delay_tolerancia_defesa / 2), "timeout")#------TEMPO DE TOLERANCIA PARA DEFESA
		$sprites_monstro.frame = 2#-----------------MONSTRO HITA
		yield(get_tree().create_timer(delay_tolerancia_defesa / 4), "timeout")
		$sprites_monstro.frame = 3
	...
	#--------------------------------------------------------------------------------------
		...
		if !monstro_vulneravel:
			$sprites_monstro.frame = 7
		yield(get_tree().create_timer(delay_ataque_1_pausa_2 / 2), "timeout")
		if !monstro_vulneravel:
			$sprites_monstro.frame = 0
...
func monstro_ataque_2():
	if !monstro_atacando:
		altera_timer(dano_1_ataque_2, delay_ataque_2_pausa_1, delay_ataque_2_hit_1)
		$Label.text = "Ataque 2 Padrao 1"
		monstro_atacando = true
		$sprites_monstro.frame = 5#----------FRAME DE AVISO VAI VIRAR 1 SEGUNDO
		yield(get_tree().create_timer(delay_ataque_2_aviso_1), "timeout")#-------DELAY DE AVISO
		$sprites_monstro.frame = 6
	
		timer_ate_hitar.start()#-------------COMECA O DELAY_ATAQUE_2_PAUSA_1
		yield(timer_ate_hitar, "timeout")
		$fire.play()
		$sprites_monstro.frame = 3#--------------MONSTRO HITA
		perfect_parry_desativado = true
	...
	#--------------------------------------------------------------------------------------
		...
		if !monstro_vulneravel:
		#------COMECA OUTRO PADRAO-------------------------------------------------
			$Label.text = "Ataque 2 Padrao 2"
			altera_timer(dano_2_ataque_2, delay_ataque_2_pausa_2, delay_ataque_2_hit_2)
			$sprites_monstro.frame = 6#---------------FRAME DE PAUSA
			timer_ate_hitar.start()#-----------COMECA O DELAY_ATAQUE_2_PAUSA_2
			yield(timer_ate_hitar, "timeout")
			$fire.play()
			$sprites_monstro.frame = 3#-------------MONSTRO HITA
			perfect_parry_desativado = true
		...
		#--------------------------------------------------------------------------------------
			...
			$Label.text = "Ataque 2 Pausa final"
			if !monstro_vulneravel:
				$sprites_monstro.frame = 7
			yield(get_tree().create_timer(delay_ataque_2_pausa_3 / 2), "timeout")
			if !monstro_vulneravel:
				$sprites_monstro.frame = 0
			yield(get_tree().create_timer(delay_ataque_2_pausa_3 / 2), "timeout")
		#------------------------------------TERMINA ATAQUE-------------------------------------------
